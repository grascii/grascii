...
def main(arguments):
    ...
    search_parser = subparsers.add_parser("search", description=search.description, help=search.description)
    search.build_argparser(search_parser)
    search_parser.set_defaults(func=search.main)

    build_parser = subparsers.add_parser("build", description=build.description, help=build.description)
