...
class TestDictionaryBuildWarnings(unittest.TestCase):
    ...
    def test_uncertainty(self):
        builder = DictionaryBuilder(infiles=[Path("tests/dictionaries/uncertainty.txt")],
    ...
    def test_count(self):
        builder = DictionaryBuilder(infiles=[Path("tests/dictionaries/count.txt")],
                                    output=self.output_dir, count_words=True)
        builder.build()
    ...
    def test_not_grascii(self):
        builder = DictionaryBuilder(infiles=[Path("tests/dictionaries/not_grascii.txt")],
                                    output=self.output_dir, parse=True)
        builder.build()
        self.assertEqual(len(builder.warnings), 0)
    ...
    def test_comments_and_whitespace(self):
        builder = DictionaryBuilder(infiles=[Path("tests/dictionaries/comments.txt")],
...
class TestBuiltins(unittest.TestCase):
    ...
    def test_preanniversary(self):
        ...
        builder = DictionaryBuilder(infiles=inputs, check_only=True, count_words=True, parse=True)
        builder.build()
        self.assertEqual(len(builder.errors), 0)
